...
from model import TwoHeadedUNet3D
from dataset import load_dataset

model_name = f"TwoHeadedUNet3D_DWI_first_{time.strftime('%Y%m%d_%H%M%S')}"
...
for epoch in range(20):
    ...
    for i, subjects in enumerate(train_dataloader):
        ...
        # load data
        images = torch.cat((subjects["dwi"]['data'], subjects["flair"]['data']), dim=1).cuda() # [B, C, W, H, D]
    ...
    with torch.no_grad():
        for i, subjects in enumerate(valid_dataloader):
            ...
            # load data
            images = torch.cat((subjects["dwi"]['data'], subjects["flair"]['data']), dim=1).cuda()
